...
class Imap {
    ...
    public function getOverview($start, $count=100) {
      $end = $start + $count;
      $range = $start . ":" . $end;
      $messages = imap_fetch_overview($this->imap, $range, FT_UID);
      return $messages;
    }
    /**
     ...
     * @param $withbody without body
     */
    public function getMessage($uid, $withbody = true) {
        $id = imap_msgno($this->imap, $uid);
        $email = $this->formatMessage($id, $withbody);
        return $email;
    }
    ...
    protected function formatMessage($id, $withbody=true){
        ...
        $headerstr = imap_fetchheader($this->imap, $id);
        ...
        foreach ($headerparts as $h) {
          $hp = explode(": ", $h, 2);
          if (count($hp) == 2) {
            $fullheader[$hp[0]] .= $hp[1];
            $lastheader = $hp[0];
          } else {
            $fullheader[$lastheader] .= "\r\n" . $h;
          }
        }
    ...
    public function deleteMessages($ids) {
        ...
        if ($this->folder == $trash) {
            if( imap_delete($this->imap, implode(",", $ids), FT_UID) == false) 
                return false;
        } else {
...
*/
        ...
        return array(
                ...
                "contentType" => $this->get_mime_type($partStruct),
                "content" => $message);
    }
    ...
    private function getAttachments($imap, $mailNum, $part, $partNum) {
        ...
        if (isset($part->disposition)) {
            if (strtolower($part->disposition) == "attachment" || strtolower($part->disposition) == "ooinline") {
                ...
                $attachmentDetails = array(
                    ...
                    "contentType" => strtolower($this->get_mime_type($part)),
                    "size"    => $part->bytes
                );
